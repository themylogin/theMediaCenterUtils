...
void set_image(char* image)
{
    ...
    DATA32* dev_im1data;
    cudaMalloc((void**)&dev_im1data, 1920 * 1080 * sizeof(DATA32));
    DATA32* dev_im2data;
    cudaMalloc((void**)&dev_im2data, 1920 * 1080 * sizeof(DATA32));
    DATA32* dev_dstData;
    cudaMalloc((void**)&dev_dstData, 1920 * 1080 * sizeof(DATA32));
    DATA32* dstData[25];
    Imlib_Image resultImage[25];
    for (int i = 0; i < 25; i++)
    {
        dstData[i] = (DATA32*)malloc(1920 * 1080 * sizeof(DATA32));
        resultImage[i] = imlib_create_image_using_data(1920, 1080, dstData[i]);
    }
    int counter = 0;
    while (1)
    {
        printf("%s -> %s\n", images[counter % images_n], images[(counter + 1) % images_n]);
        Imlib_Image im1 = imlib_load_image_immediately_without_cache(images[counter % images_n]);
        Imlib_Image im2 = imlib_load_image_immediately_without_cache(images[(counter + 1) % images_n]);
        counter++;
        
        imlib_context_set_image(im1);
        DATA32* im1data = imlib_image_get_data_for_reading_only();
        cudaMemcpy(dev_im1data, im1data, 1920 * 1080 * sizeof(DATA32), cudaMemcpyHostToDevice);
        imlib_context_set_image(im2);
        DATA32* im2data = imlib_image_get_data_for_reading_only();
        cudaMemcpy(dev_im2data, im2data, 1920 * 1080 * sizeof(DATA32), cudaMemcpyHostToDevice);
        
        for (int i = 0; i < 25; i++)
        {
            mixImages<<< 1920*1080/64, 64 >>>(dev_im2data, dev_im1data, i * 0.04, dev_dstData);
            cudaThreadSynchronize();
            cudaMemcpy(dstData[i], dev_dstData, 1920 * 1080 * sizeof(DATA32), cudaMemcpyDeviceToHost);
        }
        for (int i = 0; i < 25; i++)
        {
            imlib_context_set_image(resultImage[i]);
            imlib_render_image_on_drawable(0, 0);
            XSetWindowBackgroundPixmap(display, root, pixmap);
            XClearWindow(display, root);
            XUngrabServer(display);
...
int main(int argc, char* argv[])
{
    ...
    DATA32* srcData;
    DATA32* dstData;
    cudaMalloc((void**)&srcData, 360 * 332 * sizeof(DATA32));
    cudaMalloc((void**)&dstData, 720 * 664 * sizeof(DATA32));
    
    Imlib_Image srcImage = imlib_load_image_immediately_without_cache("/home/themylogin/Crystal Castles/http:  userserve-ak.last.fm serve _ 55302835 Crystal+Castles.jpg");
    imlib_context_set_image(srcImage);
    DATA32* srcImageData = imlib_image_get_data_for_reading_only();
    cudaMemcpy(srcData, srcImageData, 720 * 664 * sizeof(DATA32), cudaMemcpyHostToDevice);

    dim3 dimGrid(45, 42);
    dim3 dimBlock(8, 8);
    bicubicInterpolation<<< dimGrid, dimBlock >>>(srcData, 360, 332, dstData, 720, 664);
    cudaThreadSynchronize();

    DATA32* dstImageData = (DATA32*)malloc(720 * 664 * sizeof(DATA32));
    cudaMemcpy(dstImageData, dstData, 720 * 664 * sizeof(DATA32), cudaMemcpyDeviceToHost);
    Imlib_Image dstImage = imlib_create_image_using_data(720, 664, dstImageData);
    
    imlib_context_set_image(dstImage);
    imlib_render_image_on_drawable(0, 0);
    XSetWindowBackgroundPixmap(display, root, pixmap);
    XClearWindow(display, root);
    XUngrabServer(display);
    ...
    */
    ...
    /*
    int images_n = 0;
    DIR* dp = opendir("/home/themylogin/Wallpapers");
    struct dirent* de;
    ...
    char** images = (char**)malloc(images_n * sizeof(char*));
    rewinddir(dp);
    images_n = 0;
    while ((de = readdir(dp)) != NULL)
    {
        ...
        images[images_n] = (char*)malloc((strlen("/home/themylogin/Wallpapers/") + strlen(de->d_name) + 1) * sizeof(char));
        strcpy(images[images_n], "/home/themylogin/Wallpapers/");
        strcat(images[images_n], de->d_name);
        images_n++;
    }
